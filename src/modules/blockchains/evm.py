...
from src.modules import Blockchain, Logger, Client
from src.modules.interfaces import SoftwareException, SoftwareExceptionWithoutRetry
from src.utils.tools import gas_checker, helper
from config import TRANSFER_AMOUNT, LIQUIDITY_AMOUNT

from config import (
    WETH_ABI,
    TOKENS_PER_CHAIN,
    NATIVE_CONTRACTS_PER_CHAIN,
    NATIVE_ABI, CHAIN_NAME, ZKSYNC_CONTRACT_ABI, ZERO_ADDRESS,
)
...
class SimpleEVM(Logger):
    ...
    async def transfer_eth_to_myself(self):
        ...
        self.logger_msg(
            *self.client.acc_info,
            msg=f"Transfer {amount} ETH to your own {self.client.network.name} address: {self.client.address}"
        )
    ...
    async def transfer_eth(self):
        ...
        self.logger_msg(
            *self.client.acc_info, msg=f'Transfer ETH to random {self.client.network.name} address: {amount} ETH'
        )
