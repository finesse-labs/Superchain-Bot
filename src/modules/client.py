...
from src.modules.interfaces import PriceImpactException, BlockchainException, SoftwareException
...
from config import WAIT_FOR_RECEIPT
        ...
        if not check_native:
            if token_name != self.network.token:
                if token_address:
                    ...
                    print(TOKENS_PER_CHAIN2[self.network.name][token_name])
